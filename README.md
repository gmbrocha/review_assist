...
Create a real-data guarded MVP deliverable package. This runs source preparation first and fails by default if no downloaded, provided, or registered source layer is available, or if test fixture/mock source records are detected. Export and deliverable manifests include `data_lineage` and `mvp_quality` so real source evidence, source-backed constraints, inline tables/figures, placeholders, and warnings are visible:
