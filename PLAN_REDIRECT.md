...
- [x] `review-assist download-source <project_dir> epa_envirofacts_echo` exists.
...
- [x] EPA/ECHO regulated facilities downloads query the public ECHO Facilities MapServer layer 0 by project analysis bounds.
- [x] Downloaded EPA/ECHO facilities are registered as normal `local_file` sources with `status: downloaded`.
...
- [x] EPA regulated facility data where a stable public download path is practical.
...
- [x] Add source-specific label extraction rules for implemented NWI, NHD, USFWS Critical Habitat, EPA/ECHO, and FEMA downloaders.
...
- [x] Add grouped summaries by project feature and source category.
