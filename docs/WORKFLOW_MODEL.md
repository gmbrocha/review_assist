...
- `populate-for-review` runs context generation, source status resolution, source inventory generation, tolerant local spatial analysis, deterministic draft finding generation, comparison table generation, and review queue generation.
...
- It does not yet download sources, render maps, draft report prose, call LLMs, or compile exports.
...
- `generate-review-queue` creates JSON review queue items from source inventory records, deterministic draft findings, comparison tables, source status records, missing-data placeholders, spatial relationships, no-mapped-relationship checks, and validation issues.
...
- The baseline is still service/CLI only; GUI review screens, rendered map items, report drafting, and export compilation remain future work.
