...
- Deliverable matrix format: JSON at `config/deliverable_section_matrix.json`.
- Report prompt contract format: JSON at `config/report_generation_prompts.json`.
...
- Deliverable matrix config: JSON at `config/deliverable_section_matrix.json`.
- Report prompt contract config: JSON at `config/report_generation_prompts.json`.
...
- The current implementation creates draft report sections from project context, source status, source inventory, draft findings, comparison tables, map manifests, evidence packages, and validation issues. The default provider is deterministic unless `GPT_DRAFTING=1`; the GPT provider drafts only from structured evidence and stores GPT provenance/guardrail warnings. The new Sprint 1.1 deliverable matrix and report prompt contracts validate the future canonical report shape but are not wired into report generation yet.
