...
The section-level summary uses `section_need_status` to distinguish `fully_satisfied`, `partially_satisfied`, `satisfied_with_deferred`, `source_action_needed`, `deferred_source`, `manual_review_needed`, `restricted_review_needed`, `manual_reviewer_supplied`, and `not_source_backed`. `satisfied_with_deferred` preserves policy-level `deferred_source` truth when current sibling or broad-category evidence is available for screening/status context but the deferred policy need remains active.
...
The Sprint 2.1 catalog also exposes review-visible manual or stub entries for IPaC report context, state heritage review, restricted archaeology, MDEQ/manual environmental context, public water supply wells, oil wells, local businesses/economic nodes, hazardous materials support reports, and agency consultation letters. FAA airport context is now source-backed when the manually downloaded airport warehouse layer is materialized; if that source is absent, the Airports item remains an honest source-gap/review limitation rather than borrowing unrelated transportation or utility layers. These entries are not automated determinations; they keep the missing or reviewer-supplied source need visible for later deliverable tables, figures, attachments, caveats, and review items.
...
- `faa_airports`: FAA airport and aeronautical facility point context from `sources/transportation/faa_airports/raw/Airports.shp`; materialized Airports sections are screening context only and do not make airspace, obstruction, approval, or permit determinations.
...
- `usgs_wbd_huc12_subwatersheds`: USGS WBD HUC-12 Mississippi subwatershed polygons from `sources/hydrology/usgs_wbd_huc12_subwatersheds/raw/HU12.shp`; materialization keeps whole subwatershed polygons intersecting the project analysis bounds for watershed context and does not treat HUC boundaries as direct project impacts.
