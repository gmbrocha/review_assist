...
- The figure style editor can save sparse style overrides, synchronously regenerate review-only PNG figure versions under `projects/<project_id>/maps/figures/versions/<figure_id>/vN.png`, and approve one exact figure version in the figure-version artifact. These versions are visible in the editor with render-job metadata and approval state; reviewed exports use replacement figure images first, then approved versions, latest regenerated versions, and autogenerated/current generated images. Editor layer order uses presentation-only overlay z-index semantics: basemaps are lowest, source overlays default below comparison/trail features, and reviewers can lower or raise a whole editable source layer without changing source geometry or analysis.
...
- Adds compact export-facing map elements: abbreviated legend and scale bar when CRS units allow it. Deliverable figures no longer draw or advertise a compass/north arrow. Legends use a bounded map-collar/legend-bay layout that expands the visual extent enough to place the measured rendered legend inside adjacent map context, not over the core project extent or in a plain white margin. The scale bar is placed below the legend, uses alternating black/white segments, and is sized to the legend width. Report-ready PNGs are map panels only: captions, report-facing figure titles, source notes, method notes, and review/process status stay in metadata, UI chrome, and export text rather than being baked into the map canvas.
...
- Regenerated figures keep compact default legend labels for source layers. A reviewer-edited display name is used as a map legend label only when it is saved as an explicit sparse style override.
- Sprint 6.8 QA covers the core style workflow boundary: sparse override save, regenerated PNG creation, exact-version approval, reviewed export selection, reset-to-default, and later unapproved regeneration must leave source/status/context/comparison/constraint/table/evidence/review artifacts unchanged except for explicit reviewer review-state changes and expected style/version/render/export artifacts.
...
- Figure style save/reset does not approve figures, alter review/export eligibility, or regenerate figures during export. Save and Regenerate can create review-only regenerated PNG versions from saved overrides. Version approval records a project-local exact-version approval for the figure workflow; export uses approved/regenerated version artifacts only after the standard review item is already export-included.
...
6. Add legend and scale context to the map panel; preserve title, caption, source notes, method notes, and figure number as metadata/export text outside the PNG.
...
- North-arrow/compass treatment, if a future cartographic profile explicitly requires it.
- Source names.
