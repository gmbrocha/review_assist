...
- The figure style editor can save sparse style overrides, synchronously regenerate review-only PNG figure versions under `projects/<project_id>/maps/figures/versions/<figure_id>/vN.png`, and approve one exact figure version in the figure-version artifact. These versions are visible in the editor with render-job metadata and approval state; approved versions are not export-active until Sprint 6.7 export integration.
...
- Figure review detail links figure items to a presentation-only figure style editor at `/review/<item_id>/figure-style`. The editor shows the current preview, review/export status, source refs, evidence refs, validation warnings, figure caption/source/method metadata, basemap/render status, recipe/model status, editable layer list, active draft override summary, version summary, version history, and render-job summary. Comparison units are edited as individual comparison-feature rows; basemap/provenance layers are reported in status metadata but excluded from editable controls. Save/reset actions write only sparse project-local style override metadata and preserve override audit history; Save and Regenerate creates a review-only regenerated PNG version; Approve records the exact selected version without changing review queue or export state.
...
- Figure style save/reset does not approve figures, alter review/export eligibility, or apply style overrides during export. Save and Regenerate can create review-only regenerated PNG versions from saved overrides. Version approval records a project-local exact-version approval for the figure workflow, but export image selection remains deferred to Sprint 6.7.
