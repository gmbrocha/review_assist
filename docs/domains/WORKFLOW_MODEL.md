...
- The local web UI consumes the bounded standard review queue by default. Reviewer pages show human-facing item type/status labels, full reviewable generated content, table/figure previews, warnings, review actions, and export gate state without exposing raw internal IDs, source refs, artifact paths, hashes, prompt/cache fingerprints, or low-level validation codes by default.
- Existing reviewer routes accept `?advanced=1` for inline diagnostics on the same page. Advanced mode exposes the raw item/source/layer/version IDs, source/evidence refs, artifact paths, prompt/version/hash/cache fields, and QA/validation codes needed for development or audit troubleshooting. This is display filtering only; canonical queue artifacts and review/export gates are unchanged.
