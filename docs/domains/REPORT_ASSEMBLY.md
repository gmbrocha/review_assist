...
Sprint 8.1 cleaned default report-body presentation. Markdown/DOCX bodies no longer append a generated package-contents appendix, print raw source-ref lines, show raw table/figure IDs in support captions, or add map-file/figure-file path prose. Those diagnostics remain available in export manifests and advanced/debug views. Inline support tables and figures render once at their first eligible section occurrence; later sections that reference the same support table or figure remain individually visible and point back to the already-provided support instead of repeating it.
