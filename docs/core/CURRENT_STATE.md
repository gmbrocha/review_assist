...
- A developer/test reset command and local Overview action can clear generated `deliverable/deliverable_items.json` and `review_queue/review_queue.json`, then rebuild them from current artifacts for manual testing. The reset preserves source data, source registry/config, project area, comparison units, deliverable tables, deliverable figures, evidence, and exports by default unless explicit flags are used.
...
- Figure review items use a figure-specific web action form for edited captions, safe project-local replacement image uploads, and Accept Final. Export-facing figure captions/source/method notes and rendered images no longer include review/process instructions; reviewer notes remain internal review metadata.
...
- MARIS/NAIP 2025 imagery is cataloged as context-only basemap provenance. `src/review_assist/basemaps.py` indexes county folders under `sources/aerial_base_maps/maris_naip_2025`, treats `.sid` files as provenance only, recognizes `.tif`, `.tiff`, and `.png` sidecars as renderable candidates, and discovers project-local NAIP sidecars under `projects/<project_id>/basemaps/naip/`.
...
- `materialize-naip-basemap` is an explicit optional CLI command for AOI-bounded NAIP sidecar acquisition from Microsoft Planetary Computer NAIP COGs. It writes project-local GeoTIFF/JSON sidecars, reuses existing sidecars unless `--refresh` or `--force` is supplied, enforces AOI/tile/pixel/time limits, resamples oversized native-resolution windows to fit the configured output-pixel cap, and records controlled failure status without changing normal populate behavior.
...
- Deliverable figure legends now use a bounded map-collar layout when needed: the rendered extent expands slightly on a low-conflict side so the legend sits in an adjacent map bay rather than over project features or in a large outside margin. Source layer symbology is lighter than project/comparison-unit symbology.
