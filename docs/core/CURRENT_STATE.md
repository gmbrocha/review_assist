...
- Completed: Sprint 6.3 Reviewer Figure Editor UI.
- Completed: Sprint 6.4 Style Override Save Logic.
- Active next implementation target: Sprint 6.5 Regeneration Job.
...
- Figure review detail now links figure items to a presentation-only figure style editor at `/review/<item_id>/figure-style`. The editor displays the current figure preview, review/export status, source refs, evidence refs, validation warnings, figure caption/source/method metadata, basemap/render status, recipe/model status, layer list, selected-layer controls, active draft override summary, version summary, and render-job summary. Non-figure review items cannot open the editor. Save/reset actions persist or clear sparse draft style overrides only; regeneration, new figure versions, figure approval, export integration, review queue mutation, and rendered image replacement remain deferred.
- Figure style overrides validate the Sprint 6.2 snake_case v1 layer fields, reject unknown fields/layers and out-of-range values, preserve an audit history of superseded/reset records, and default local edits to actor `local_reviewer`. Saving unchanged defaults leaves no active draft override.
...
- MARIS/NAIP 2025 imagery is cataloged as context-only basemap provenance. `src/review_assist/basemaps.py` can index county metadata under `sources/aerial_base_maps/maris_naip_2025`, but real MrSID `.sid`/`.sdw` payloads are not retained as active Review Assist assets. The basemap workflow recognizes `.tif`, `.tiff`, `.png`, `.jpg`, and `.jpeg` files as renderable candidates and discovers project-local NAIP render assets under `projects/<project_id>/basemaps/naip/`, including grouped figure-extent assets.
...
- NAIP/MARIS basemap provenance and renderability are recorded in `project_area.json` and source status detail. Deliverable figure rendering can use selected renderable sidecars or project-local NAIP assets; legacy MrSID references are unsupported/stale provenance and are not active render assets. `populate-for-review --materialize-naip-basemap` can opt into failure-tolerant grouped NAIP sidecar materialization for planned figure render extents; plain populate does not acquire imagery.
