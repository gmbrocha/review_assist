...
- Deliverable figure rendering now styles comparison units individually with saturated, high-separation deterministic colors. KML colors are used only when saturated/readable on aerial imagery and distinct from already assigned comparison-unit colors; otherwise the renderer uses stable high-contrast fallbacks without the old heavy white casing. This does not change comparison-unit geometry or constraint analysis.
