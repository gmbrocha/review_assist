...
- Active next implementation target: Sprint 6.2 Figure Recipe And Override Model.
...
- Sprint 6 has started as a constrained reviewer figure-styling workflow. Sprint 6.1 audited the existing figure generation/review/export flow and confirmed the immutable analytical boundaries for style-only edits. Sprint 6.2 is active next and should add project-local recipe, style override, render job, and figure version metadata without changing figure rendering, export resolution, source truth, evidence counts, report policy, GPT behavior, or review-gate semantics.
